...
GraphIQ는 이 공시 데이터를 **그래프 데이터베이스(Neo4j)에 적재**하고, 인터랙티브 시각화와 자연어 채팅 인터페이스를 통해 누구나 쉽게 탐색할 수 있도록 합니다.
...
- **채팅-쿼리 분리** — 자연어 이해(LLM)와 데이터 조회(Cypher)를 레이어로 분리해 각각 독립적으로 개선 가능
